...
class Dataset_Pro(data.Dataset):
    def __init__(self, file_path_r, file_path_t, is_train=1, ir=1, SNR=15, is_U2D=0, is_few=0,
                 train_per=0.9, valid_per=0.1, use_deepmimo_data=False):
        super(Dataset_Pro, self).__init__()
        self.SNR = SNR
        self.ir = ir

        if use_deepmimo_data:
            H_his = hdf5storage.loadmat(file_path_r)['c_up_his']  # v,b,l,k,a,b,c
            if is_U2D:
                H_pre = hdf5storage.loadmat(file_path_t)["c_down_pre"]  # v,b,l,k,a,b,c
            else:
                H_pre = hdf5storage.loadmat(file_path_t)["c_up_pre"]  # v,b,l,k,a,b,c
            # print(H_his.shape, H_pre.shape)

            H_his = rearrange(H_his, 'a b c -> c a b')  # 145000, 16, 48
            H_pre = rearrange(H_pre, 'a b c -> c a b')
            ...
            H_his = H_his / std
            H_pre = H_pre / std
            H_pre = LoadBatch_ofdm_deepmimo(H_pre)
            H_his = LoadBatch_ofdm_deepmimo(H_his)
            if is_few == 1:
                H_pre = H_pre[::10, ...]
                H_his = H_his[::10, ...]
...
def LoadBatch_ofdm_deepmimo(H):
    B, T, m = H.shape
    # H = rearrange(H, 'b t (k a) ->(b a) t k', a=num)
    H_real = np.zeros([B, T, m, 2])
    H_real[:, :, :, 0] = H.real
    H_real[:, :, :, 1] = H.imag
    H_real = H_real.reshape([B, T, m * 2])
